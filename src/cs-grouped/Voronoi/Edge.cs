  ...
  //*/
	...
	internal virtual void outputVoronoiDiagram()
	{
		...
		do
		{
			...
			vv2 = vv1.times(d1);
			vv3 = center.sum(vv2);
			System.Console.Out.WriteLine("Vedge " + center.ToString() + " " + vv3.ToString());
			nex = nex.rNext();
		} while (nex != this);
		...
		pushRing(edges, seen);
		System.Console.Out.WriteLine("no. of edges = " + edges.Count);
		while (!(edges.Count == 0))
		{
			Edge edge = (Edge)edges.Pop();
			bool b = (bool)seen[edge];
			if (b != null && b)
			{
				Edge prev = edge;
				nex = edge.oNext();
				do
				{
					...
					if (d1 >= d2)
					{
						System.Console.Out.WriteLine("Dedge " + v1 + " " + v2);
						...
						if (v1.ccw(v2, v3) != v1.ccw(v2, v4))
						{
							...
							System.Console.WriteLine("Vedge " + vv1.ToString() + " " + vv2.ToString());
						}
					}
					seen[prev] = false;
					prev = nex;
