...
f_1_1178_1278("Treeadd alloc time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end0.Subtract(start0).TotalMilliseconds) / 1000.0).ToString(),1,1227,1277));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(1,1284,1383);

f_1_1284_1382("Treeadd add time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start1).TotalMilliseconds) / 1000.0).ToString(),1,1331,1381));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(1,1388,1489);

f_1_1388_1488("Treeadd total time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start0).TotalMilliseconds) / 1000.0).ToString(),1,1437,1487));
