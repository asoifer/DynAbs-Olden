...
f_1_1989_2076("Creation time: " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end2.Subtract(start2).TotalMilliseconds)/1000.0).ToString(),1,2027,2075));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(1,2089,2186);

f_1_2089_2185("Time to sort forward = " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end0.Subtract(start0).TotalMilliseconds) /1000.0).ToString(),1,2135,2184));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(1,2198,2296);

f_1_2198_2295("Time to sort backward = " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start1).TotalMilliseconds) /1000.0).ToString(),1,2245,2294));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(1,2302,2383);

f_1_2302_2382("Total: " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start0).TotalMilliseconds) /1000.0).ToString(),1,2332,2381));
