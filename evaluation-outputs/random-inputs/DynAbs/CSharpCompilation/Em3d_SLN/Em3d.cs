...
f_2_1961_2049("EM3D build time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end0.Subtract(start0).TotalMilliseconds)/1000.0).ToString(),2,2000,2048));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(2,2056,2147);

f_2_2056_2146("EM3D compute time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start1).TotalMilliseconds)/1000.0).ToString(),2,2097,2145));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(2,2153,2242);

f_2_2153_2241("EM3D total time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start0).TotalMilliseconds)/1000.0).ToString(),2,2192,2240));
