...
f_5_1826_1918("QuadTree alloc time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end0.Subtract(start0).TotalMilliseconds)/1000.0).ToString(),5,1869,1917));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(5,1925,2016);

f_5_1925_2015("Count leaves time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end1.Subtract(start1).TotalMilliseconds)/1000.0).ToString(),5,1966,2014));
DynAbs.Tracing.TraceSender.TraceSimpleStatement(5,2022,2118);

f_5_2022_2117("Perimeter compute time " + DynAbs.Tracing.TraceSender.TraceInvocationWrapper(() => ((end2.Subtract(start2).TotalMilliseconds)/1000.0).ToString(),5,2068,2116));
